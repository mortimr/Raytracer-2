/*
** menu_cub_end.c for RTV2 in /home/rotaru_i/gfx_raytracer2/src
**
** Made by
** Login   <rotaru_i@epitech.net>
**
** Started on  Sun May 22 20:13:32 2016
** Last update Sun May 22 20:27:19 2016 
*/
		     ...
		     t_data *data)
{
  ...
  if ((state == GO_DOWN && butt == BMB_LEFT)
      && (cl_btw(150, 265, 450, 496) == OK))
    start_edit(data, 4, &data->menu.fig_focus->size.x);
  if ((state == GO_DOWN && butt == BMB_LEFT)
      && (cl_btw(265, 380, 450, 496) == OK))
    start_edit(data, 4, &data->menu.fig_focus->size.y);
  if ((state == GO_DOWN && butt == BMB_LEFT)
      && (cl_btw(380, 500, 450, 496) == OK))
    start_edit(data, 4, &data->menu.fig_focus->size.z);
}
		       ...
		       t_data *data)
{
  ...
  menu_cub_mid(state, butt, data);
  menu_cub_end(state, butt, data);
}
		     ...
		     t_data *data)
{
  ...
  if ((state == GO_DOWN && butt == BMB_LEFT)
    ...
    start_edit(data, 3, &data->menu.fig_focus->ambiant);
}
...
{
  if (data->menu.focus != 1 || !data->menu.fig_focus
      || data->menu.fig_focus->type != LIGHT
      || data->menu.menu_status != 2 || data->add == 1)
    return ;
  if ((state == GO_DOWN && butt == BMB_LEFT && data->menu.fig_focus)
      && (cl_btw(275 - (len(data->menu.fig_focus->name) / 2) * 30,
